...
class BorrowerProfileController extends Controller
{
    ...
    {
        // Récupérer le profil emprunteur avec ses emprunts
        $borrowerProfile = BorrowerProfile::with(['borrows'])->findOrFail($id);
        ...
        $borrowedInPeriod = $borrowerProfile->borrows()
            ->whereBetween('borrow_date', [$startDate, $endDate])
            ->paginate(10);

        // Statistiques des emprunts par période
        $totalBorrowed = $borrowedInPeriod->count();
        $totalReturned = $borrowedInPeriod->filter(function ($borrow) {
            return !is_null($borrow->return_date);
        })->count();

        $currentBorrows = $totalBorrowed - $totalReturned;

        // Statistiques supplémentaires pour cette période
        ...
        return view('borrower_profiles.show', compact('borrowerProfile', 'borrowedInPeriod', 'stats', 'period'));
    }
